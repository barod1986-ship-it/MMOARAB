...
/**
 * Popular Games widget class.
 */
class MCP_Widget_Popular extends \WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'mcp_popular_games',
			__( 'الألعاب الشائعة', 'momarab-core' ),
			array(
				'description' => __( 'عرض الألعاب الأعلى تقييماً', 'momarab-core' ),
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'الألعاب الشائعة', 'momarab-core' );
		$number = ! empty( $instance['number'] ) ? absint( $instance['number'] ) : 5;
		$number = min( $number, 50 ); // Enforce max 50.

		// Get cached results.
		$cache_key = 'mcp_widget_popular_' . $number;
		$games = get_transient( $cache_key );

		if ( false === $games ) {
			$games = $this->get_popular_games( $number );
		...
		echo '<ul class="mcp-popular-games-list">';
	 ...
	 */
	private function get_popular_games( $number ) {
		$query = new \WP_Query( array(
			'post_type'      => 'games',
			'post_status'    => 'publish',
			'posts_per_page' => $number,
			'meta_key'       => 'mcp_rating_overall',
	...
	private function render_game_item( $game ) {
		?>
		<li class="mcp-popular-game-item">
	...
	private function clear_cache() {
		...
		$transients = $wpdb->get_results(
			"SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE '_transient_mcp_widget_popular_%'"
		);
